...
class Requestor:
    ...
    def fetch_raw_data(self) -> None:
        ...
        response_part3 = (
            response_part2.find(id="innerLayout")
            .find(id="listContainer")
            .find(id="body-container")
            .find(id="gallerywide")
            .find_all("li", {"class": "wrap"})
        )
    ...
    def send_email(self, new_flats: Dict) -> None:
        ...
        try:
            session = smtplib.SMTP_SSL(
                "smtp.poczta.onet.pl", 465, context=context, timeout=120
            )
            session.ehlo()
    ...
    def load_repository(self) -> None:
        ...
        try:
            dataframe = pandas.read_csv(
                filepath_or_buffer="./repository.csv", index_col=0
            )
    ...
    def save_repository(self) -> None:
        dataframe = pandas.DataFrame.from_dict(
            data=self.flat_repository,
            orient="index",
            columns=["title", "url", "fetch_date", "price"],
        )
...
def execute_requestor_job(requestor: Requestor):
    ...
    for potential_new_flat_id in requestor.enriched_data.keys():
        if not potential_new_flat_id in requestor.flat_repository.keys():
            new_flat = {
                potential_new_flat_id: requestor.enriched_data[potential_new_flat_id]
            }
...
if __name__ == "__main__":
    ...
    while True:
        print(
            f'--{datetime.datetime.now().strftime("%m/%d/%Y, %H:%M:%S")} -- Start of iteration. '
        )
        ...
        print(
            f'--{datetime.datetime.now().strftime("%m/%d/%Y, %H:%M:%S")} -- End of iteration. Loop time: {timer_result} sec. '
        )
